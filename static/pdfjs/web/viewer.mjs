  ...
  load(pdfDocument) {
    ...
    firstPagePromise.then(pdfPage => {
      ...
      Promise.all([animationStarted, storedPromise, pageLayoutPromise, pageModePromise, openActionPromise]).then(async ([timeStamp, stored, pageLayout, pageMode, openAction]) => {
        ...
        // Custom: Read spread mode from hash parameter
        const hashParams = parseQueryString(document.location.hash.substring(1));
        if (hashParams.has("spread")) {
          const spreadParam = hashParams.get("spread");
          if (spreadParam === "none") {
            spreadMode = SpreadMode.NONE;
          } else if (spreadParam === "odd") {
            spreadMode = SpreadMode.ODD;
          } else if (spreadParam === "even") {
            spreadMode = SpreadMode.EVEN;
          }
        }
...
}
{
  ...
  var validateFileURL = function (file) {
    // Origin validation disabled to allow loading PDFs from Storyblok CDN
    return;
